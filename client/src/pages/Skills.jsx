    ...
    <PageTransition>
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {skills.map((cat, idx) => (
            <AnimatedSection key={cat.id} variant="scaleUp" delay={idx * 0.08}>
              <motion.div
                whileHover={{ y: -8, scale: 1.02 }}
                transition={{ type: "spring", stiffness: 300 }}
                className="group h-full bg-[#001428]/80 border border-cyan-500/15 rounded-2xl p-6 text-center hover:shadow-xl hover:shadow-cyan-500/10 hover:border-cyan-500/40 transition-colors duration-500 relative overflow-hidden"
              >
                ...
                <div className="relative">
                  ...
                  <div className="flex flex-wrap justify-center gap-2">
                    {cat.items?.map((item, j) => (
                      <motion.span
                        key={j}
                        whileHover={{ scale: 1.15, boxShadow: "0 0 15px rgba(0,200,255,0.3)" }}
                        className="bg-cyan-400/10 text-cyan-300 text-xs px-3 py-1.5 rounded-full border border-cyan-500/20 hover:bg-cyan-400/25 transition-colors duration-200 cursor-default"
                      >
                        {item}
                      </motion.span>
                    ))}
